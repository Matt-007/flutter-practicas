...
class ActualizarClientesScreen extends StatefulWidget {
  final String token;
  final int clientId; // Nuevo parámetro
...
class _ActualizarClientesScreenState extends State<ActualizarClientesScreen> {
  ...
  // Variable para mantener el ID del cliente a actualizar
  int? _clientId;
  ...
  void _actualizarCliente() async {
    if (_clientId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Por favor, busque un cliente primero')),
      );
      return;
    }

    final url = Uri.parse(
        'https://aibootbackend.sistemaagil.net/api/bpartner/$_clientId');
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                int clientId = int.tryParse(_clientIdController.text) ?? -1;
                if (clientId > 0) {
                  _fetchClientData(clientId);
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('ID del Cliente inválido')),
                  );
                }
              },
